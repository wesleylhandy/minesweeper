...
!function(global) {
    ...
    class StoredTimes {
        ...
        init() {
            ...
            if (this.root) return
            this.root = document.getElementById(`best-time-${this.gameType.toLowerCase()}`)
        }
        ...
        getBestTimeFromStorage() {
            ...
            this.bestTime = bestTime < Infinity ? Timer.toTimeString(bestTime) : "♾️"
        }
        ...
        setAndStoreBestTime(timeElapsed) {
            ...
            if (storedTimes) {
                ...
                if (!parsed[this.gameType]) {
                    ...
                    parsed[this.gameType] = timeElapsed < parsed[this.gameType] ? timeElapsed : parsed[this.gameType]
                }
                localStorage.setItem("bestTimes", JSON.stringify(parsed))
                this.bestTime = parsed[this.gameType] < Infinity ? Timer.toTimeString(parsed[this.gameType]) : "♾️"
            } else {
                const bestTimes = {}
                ...
                this.bestTime = bestTimes[this.gameType] < Infinity ? Timer.toTimeString(bestTimes[this.gameType]) : "♾️"
            }
            this.render()
        }
    ...
    class Game {
        ...
        reset(gameType) {
            ...
            this.root.innerHTML = ""
            this.root.classList.remove("game-over", GAME_OVER_CLASSES.WIN, GAME_OVER_CLASSES.LOSE)
    ...
    class Flags {
        ...
        render() {
            this.root.textContent = `${this.maxFlags - this.flagsUsed} 🚩`
        }
    }
        ...
        fillGridWithMines() {
            ...
            while (mines < this.numMines) {
                ...
                if (this.grid[rowCoord][colCoord].value !== MINE) {
                    ...
                    if (rowCoord + 1 !== this.numRows && colCoord + 1 !== this.numColumns) {  
        ...
        checkForWin() {
            ...
            for (let row = 0; row < this.numRows; row++) {
                for (let col = 0; col < this.numColumns; col++) {
                    if (!this.grid[row][col].revealed && this.grid[row][col].value !== MINE) {
                        win = false
                        break;
                    }
                }
    ...
    class Square {
        ...
        handleClick(e) {
            e.preventDefault()
            if (e.button === 2 || e.which === 3 || e.ctrlKey) {
                this.toggleFlag()
                return
            }
            this.handleReveal()
        ...
        toggleFlag() {
            ...
            if (((this.parent.flags.flagsUsed < this.parent.flags.maxFlags && !this.flagged) || this.flagged) && !this.revealed) {
                this.flagged = !this.flagged
                this.root.textContent = this.flagged ? "🚩" : ""
        ...
        removeClickHandler() {
            ...
            this.root.removeEventListener("contextmenu", this.handleRightClick.bind(this))
        }

        revealSquare() {
            this.revealed = true;
            const content = this.value === 0 ? "" : this.value;
            this.root.textContent = this.value === MINE ? "💣" : content
